...
| 遇到障碍停下后就不动了 | 障碍物压在路径点上，且刹车距离(4m) > 到达半径(3m)，车永远走不到那个点 | 已修复（2026-09-26）：测试台被挡超 2 秒会自动绕行。也可手动把 `Brake Distance` 调到 3 以下 |
...
## 附：为什么"遇到障碍就停住不动"（2026-09-26 修复）

这是 B 模块第一个真正的 bug，值得你理解清楚。

原因是一道几何题：

- 障碍物 `Obstacle_正面` 被摆在 `WP3` 的**正上方**
- 车距障碍物 **4 米**就刹停（`Brake Distance`）
- 但要走目标点 **3 米**以内才算"到达"、才切下一个点（`Arrival Radius`）

**4 > 3，所以车永远走不进那个 3 米圈** —— 它被自己的刹车规则挡在门外了。
而且此时目标点就在车头正前方，`Steering` 算出来约等于 0，车既不前进也不转向，就那么僵住。

修复分两层：

1. `WaypointFollower` 新增 `targetOffset`（临时目标点偏移），对外接口**没有变**
2. `BTestRig` 新增卡死检测：被挡超过 `Stuck Timeout`（2 秒）就把目标点往车身侧面挪 `Detour Offset`（3.5 米），
   持续 `Detour Duration`（2.5 秒）后回到原路径。这次往右绕不过去，下次自动换左边

**这条经验可以推广到所有控制逻辑**：任何"停车条件"和"到达条件"必须能同时满足，否则就是死锁。
报告里"局限与改进"那一节可以写这个例子。
